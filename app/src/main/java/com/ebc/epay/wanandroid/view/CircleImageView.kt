package com.ebc.epay.wanandroid.view
...
/**
 * Created by ocean on 2019-12-12
 * @describe:
 */
class CircleImageView@JvmOverloads constructor(context: Context?, attrs: AttributeSet?,int: Int=0) : AppCompatImageView(context, attrs,int) {

    private var mType: Int=DEFAULT_TYPE
    private var mBorderColor: Int=DEFAULT_BORDER_COLOR
    private var mBorderWidth: Int=dip2px(DEFAULT_BORDER_WIDTH)
    private var mRectRoundRadius: Int=dip2px(DEFAULT_RECT_ROUND_RADIUS)
    ...
    init {
        ...
        ta?.run {
            mType = getInt(R.styleable.CircleImageView_type, DEFAULT_TYPE)
            mBorderColor = getColor(R.styleable.CircleImageView_borderColor, DEFAULT_BORDER_COLOR)
            mBorderWidth = getDimensionPixelSize(R.styleable.CircleImageView_borderWidth, dip2px(DEFAULT_BORDER_WIDTH))
            mRectRoundRadius = getDimensionPixelSize(R.styleable.CircleImageView_rectRoundRadius, dip2px(DEFAULT_RECT_ROUND_RADIUS))
            recycle()
        }
